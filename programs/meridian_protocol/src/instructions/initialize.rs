...
use crate::states::{LendingPool,AdminRegistry};
...
impl<'info> Initialize<'info>{
    ...
    loan_to_value_bps: u16,  utilization_rate_tier_1_bps: u16,
 ...
 pub fn log_state(&mut self){
    ...
    println!("Owner of the lending pool is: {}",self.lending_pool.owner.key());
    println!(" Address of the admin Registry of the pool is : {}", self.admin_registry.key());
    println!("Mint of the usdc is : {} ", self.lending_pool_usdc_ata.key());
    println!("Address of the protocol's usdc ata is: {}", self.lending_pool.protocol_usdc_vault);
    println!("Address of the protocol's fee vault is: {}", self.lending_pool.protocol_fee_vault);
    println!("Liquidation threshold currently is at: {} bps", self.lending_pool.liquidation_threshold_bps);
    
    println!("Liquidator rewards(bps) currently is at: {}", self.lending_pool.liquidator_reward_bps);
    println!("Liquidator penalty(bps) currently is at: {}",self.lending_pool.liquidation_penalty_bps);
    println!("Current Loan To Value(bps) currently is at: {}",self.lending_pool.loan_to_value_bps);
